...
import { isSquareAttacked } from "./check-square-attacked";
...
export const checkQueen = (
    boardState: BoardState,
    cordinate: Square,
    chosenSquare: Square
) => {
    return (
        checkBishop(boardState, cordinate, chosenSquare) ||
        checkRook(boardState, cordinate, chosenSquare)
    );
};
...
export const checkKing = (
    ...
    const color = getColor(boardState[chosenSquare.rank][chosenSquare.file]);

    if (color !== null && isSquareAttacked(boardState, cordinate, color)) {
        return false;
    }
    return (
